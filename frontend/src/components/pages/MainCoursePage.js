...
const MainCoursePage = () => {
    ...
    const {getAllTaskAndLektion, getAllTaskAndLektionForTeacher, refreshToken, error, loading, clearError} = useUserService();
    ...
    function createList(taskAndLektionsList) {
        if (teacher) {
            const opened = taskAndLektionsList.length !== 0 ? taskAndLektionsList.map((item, i) => {
                if (item.opened === true) {
                    return (
                        <div className='container-main-course-page-cart'>
            ...
            return taskAndLektionsList.length !== 0 ? <><h1>Открытые</h1>{opened}<h1>Закрытые</h1>{closed}</> : <h1>Заданий пока нет</h1>;
